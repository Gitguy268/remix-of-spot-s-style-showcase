...
import { useProgress, Html, useGLTF, ContactShadows, OrbitControls } from "@react-three/drei";
...
const GLBModel = ({ url, isRotating }: { url: string; isRotating: boolean }) => {
  ...
  useFrame((state) => {
    if (groupRef.current && isRotating) groupRef.current.rotation.y += 0.005;
    if (groupRef.current) groupRef.current.position.y = Math.sin(state.clock.elapsedTime * 0.5) * 0.03;
  });
...
const Spot3DViewer = () => {
  ...
  const controlsRef = useRef<{ reset: () => void; dollyIn: (s: number) => void; dollyOut: (s: number) => void; update: () => void } | null>(null);
        ...
        <div className="section-container relative z-10">
          <AnimatedSection animation="fade-up">
            <div className="text-center mb-12">
              <h2 className="text-4xl md:text-5xl font-bold mb-4">Meet Spot in <span className="text-gradient">3D</span></h2>
  ...
  return (
    <section className="py-24 relative" aria-labelledby="3d-viewer-heading">
      <div className="section-container relative z-10">
        <AnimatedSection animation="fade-up">
          <div className="text-center mb-12">
            <h2 id="3d-viewer-heading" className="text-4xl md:text-5xl font-bold mb-4">Meet Spot in <span className="text-gradient">3D</span></h2>
            <p className="text-lg text-muted-foreground max-w-xl mx-auto">Explore our products from every angle. Drag to rotate, scroll to zoom.</p>
          </div>
        </AnimatedSection>
          ...
          <div className="relative max-w-4xl mx-auto">
            <div className="aspect-square md:aspect-video rounded-2xl overflow-hidden glow-border bg-card relative">
              <ErrorBoundary
                ...
                onError={(error) => { console.error('3D Viewer Error:', error); setHasError(true); }}
              >
            ...
            <div className="absolute top-4 right-4 flex flex-col gap-2">
              <Button variant="outline" size="icon" onClick={() => setIsRotating(!isRotating)} className="bg-background/80 backdrop-blur-sm" aria-label={isRotating ? "Stop auto-rotation" : "Start auto-rotation"}>
                <RotateCcw className={`w-4 h-4 ${isRotating ? "animate-spin" : ""}`} style={{ animationDuration: "3s" }} />
              </Button>
              <Button variant="outline" size="icon" onClick={handleZoomIn} className="bg-background/80 backdrop-blur-sm" aria-label="Zoom in"><ZoomIn className="w-4 h-4" /></Button>
              <Button variant="outline" size="icon" onClick={handleZoomOut} className="bg-background/80 backdrop-blur-sm" aria-label="Zoom out"><ZoomOut className="w-4 h-4" /></Button>
              <Button variant="outline" size="icon" onClick={handleReset} className="bg-background/80 backdrop-blur-sm" aria-label="Reset view"><Maximize2 className="w-4 h-4" /></Button>
            </div>

            <div className="flex flex-wrap justify-center gap-3 mt-6">
              {defaultModels.map((model) => (
                <button key={model.id} onClick={() => setActiveModel(model.id)} className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-background ${activeModel === model.id ? "bg-primary text-primary-foreground" : "bg-muted/10 text-muted-foreground hover:bg-muted/20"}`}>
                  <Box className="w-4 h-4" />{model.name}
                </button>
              ))}
            </div>
            <p className="text-center text-sm text-muted-foreground mt-6">Click and drag to rotate • Scroll to zoom • Use controls for precision</p>
          </div>
        </AnimatedSection>
      </div>
    </section>
