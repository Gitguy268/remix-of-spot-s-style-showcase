...
const BASE2: RGB = { r: 225, g: 241, b: 242 };
const BLUE_SHADES: RGB[] = [
  { r: 24, g: 66, b: 175 }, { r: 34, g: 99, b: 210 }, { r: 15, g: 70, b: 190 },
  { r: 60, g: 120, b: 220 }, { r: 40, g: 140, b: 235 },
];
const PARTICLE_COUNT = 120;
const ORB_COUNT = 4;

class Particle {
  x = 0; y = 0; z = 0; ox = 0; oy = 0; size = 0; speedZ = 0;
  phase = 0; amp = 0; freq = 0; blueIndex = 0; brightness = 0;
  constructor() { this.reset(true); }
  reset(randomizePos = false) {
    this.x = (Math.random() - 0.5) * 2000; this.y = (Math.random() - 0.5) * 2000;
    this.z = Math.random() * 2000; this.size = Math.random() * 1.5 + 0.6;
    this.speedZ = Math.random() * 0.9 + 0.25; this.phase = Math.random() * Math.PI * 2;
    this.amp = Math.random() * 18 + 8; this.freq = Math.random() * 0.015 + 0.005;
  ...
  update(time: number, mouseX: number, mouseY: number) {
    this.z -= this.speedZ;
    if (this.z <= 0) { this.z = 2000; this.ox = (Math.random() - 0.5) * 2000; this.oy = (Math.random() - 0.5) * 2000; }
    ...
    const mx = (mouseX - 0.5) * 320; const my = (mouseY - 0.5) * 320;
    const depthFactor = 1 - this.z / 2000;
    this.x += mx * depthFactor; this.y += my * depthFactor;
  }
  draw(ctx: CanvasRenderingContext2D, width: number, height: number, blend: number) {
    const perspective = 900; const scale = perspective / (perspective + this.z);
    const sx = width / 2 + this.x * scale; const sy = height / 2 + this.y * scale;
    ...
    ctx.beginPath(); ctx.arc(sx, sy, radius * 2.2, 0, Math.PI * 2);
    ctx.fillStyle = grad; ctx.fill();
  }
}

class FloatingOrb {
  x = 0; y = 0; z = 0; radius = 0; phase = 0; vx = 0; vy = 0; blueIndex = 0; pulse = 0;
  constructor() {
    this.x = Math.random() * 2000 - 1000; this.y = Math.random() * 2000 - 1000;
    this.z = Math.random() * 1500 + 500; this.radius = Math.random() * 110 + 70;
  ...
  update(mouseX: number, mouseY: number) {
    ...
    if (this.x > 1500) this.x = -1500; if (this.x < -1500) this.x = 1500;
    if (this.y > 1500) this.y = -1500; if (this.y < -1500) this.y = 1500;
    this.x += (mouseX - 0.5) * 140 * 0.012; this.y += (mouseY - 0.5) * 140 * 0.012;
  }
  draw(ctx: CanvasRenderingContext2D, width: number, height: number, time: number, blend: number) {
    ...
    const blue = BLUE_SHADES[this.blueIndex];
    const r = Math.round(BASE2.r + (blue.r - BASE2.r) * blend);
    const g = Math.round(BASE2.g + (blue.g - BASE2.g) * blend);
    const b = Math.round(BASE2.b + (blue.b - BASE2.b) * blend);
...
const ParticleBackground = () => {
  ...
  const stateRef = useRef({
    particles: [] as Particle[], orbs: [] as FloatingOrb[],
    time: 0, mouseX: 0.5, mouseY: 0.5, targetMouseX: 0.5, targetMouseY: 0.5,
    blend: 0, targetBlend: 0, lastInputTime: performance.now(), width: 0, height: 0,
  });
  ...
  const drawBackground = useCallback((ctx: CanvasRenderingContext2D, mouseX: number, mouseY: number, blend: number) => {
    ...
    const grad = ctx.createRadialGradient(width * mouseX, height * mouseY, 0, width / 2, height / 2, Math.max(width, height));
  ...
  const drawVignette = useCallback((ctx: CanvasRenderingContext2D) => {
    const { width, height } = stateRef.current;
    const grad = ctx.createRadialGradient(width / 2, height / 2, height * 0.33, width / 2, height / 2, Math.max(width, height) * 0.85);
  ...
  useEffect(() => {
    ...
    const init = () => {
      resize();
      state.particles = []; state.orbs = [];
      for (let i = 0; i < PARTICLE_COUNT; i++) state.particles.push(new Particle());
      for (let i = 0; i < ORB_COUNT; i++) state.orbs.push(new FloatingOrb());
    };
    ...
    const animate = () => {
      ...
      state.orbs.forEach(o => { o.update(state.mouseX, state.mouseY); o.draw(ctx, state.width, state.height, state.time, state.blend); });

      if (state.time % 60 === 0) {
        state.particles.sort((a, b) => b.z - a.z);
      }
      state.particles.forEach(p => { p.update(state.time, state.mouseX, state.mouseY); p.draw(ctx, state.width, state.height, state.blend); });
    ...
    const updateBlendFromPointer = (nx: number, ny: number) => {
      state.targetMouseX = nx; state.targetMouseY = ny;
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      updateBlendFromPointer((e.clientX - rect.left) / rect.width, (e.clientY - rect.top) / rect.height);
    };
    const handleMouseLeave = () => { state.targetMouseX = 0.5; state.targetMouseY = 0.5; state.targetBlend = 0; };
    const handleTouchMove = (e: TouchEvent) => {
      if (e.touches.length > 0) {
        const rect = canvas.getBoundingClientRect(); const t = e.touches[0];
        updateBlendFromPointer((t.clientX - rect.left) / rect.width, (t.clientY - rect.top) / rect.height);
      }
    };
    const handleTouchEnd = () => { state.targetBlend = 0; };
    ...
    return () => {
      cancelAnimationFrame(animationRef.current); resizeObserver.disconnect();
